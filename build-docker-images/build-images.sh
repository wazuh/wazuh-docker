...
build() {
    ...
    # WAZUH_MINOR_VERSION: Extracts major and minor version only (e.g., 9.9.9 -> 5.0)
    WAZUH_MINOR_VERSION="${WAZUH_IMAGE_VERSION%.*}"
    # WAZUH_MAJOR_VERSION: Extracts major version only (e.g., 9.9.9 -> 5)
    ...
    #   dev=false, tag=9.9.9       → 9.9.9
    #   dev=false, tag=9.9.9-beta1 → 9.9.9-beta1
    #   dev=true,  tag=9.9.9       → 9.9.9-latest
    #   dev=true,  tag=9.9.9-beta1 → 9.9.9-beta1-latest
    make_tag() {
        local commit=$1
        if [ -n "${IS_DEV_BUILD}" ]; then

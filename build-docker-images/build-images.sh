...
build() {
    ...
    # WAZUH_MINOR_VERSION: Extracts major and minor version only (e.g., 10.9.9 -> 5.0)
    WAZUH_MINOR_VERSION="${WAZUH_IMAGE_VERSION%.*}"
    # WAZUH_MAJOR_VERSION: Extracts major version only (e.g., 10.9.9 -> 5)
    ...
    for component in "${components_to_build[@]}"; do
        ...
        #   dev=false, tag=10.9.9       → 10.9.9
        #   dev=false, tag=10.9.9-alpha0 → 10.9.9-alpha0
        #   dev=true,  tag=10.9.9       → 10.9.9-latest
        #   dev=true,  tag=10.9.9-alpha0 → 10.9.9-alpha0-latest
        if [ -n "${IS_DEV_BUILD}" ]; then

...
This section provides detailed instructions for deploying Wazuh-Docker (version 4.14.4) in various configurations. Choose the deployment model that best suits your needs, from simple single-node setups for testing to more robust multi-node configurations for production environments.
...
-   Cloned the `wazuh-docker` repository (version `4.14.4`) or downloaded the necessary deployment files.
    ```bash
